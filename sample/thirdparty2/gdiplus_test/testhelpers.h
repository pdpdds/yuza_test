...
ATTRIBUTE_USED static void assertEqualStringImpl(const WCHAR *actual, const char * expected, const char *message, const char *file, const char *function, int line)
{
    ...
    {
        char *actualA = charFromWchar ((const gunichar2 *)actual);
...
{
    gunichar2* wImage = createWchar((const gchar*)"test-image.png");
	GpStatus status = GdipSaveImageToFile (image, (WCHAR*)wImage, &pngEncoderClsid, NULL);
	assertEqualInt (status, Ok);
